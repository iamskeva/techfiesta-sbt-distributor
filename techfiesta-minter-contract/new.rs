...
//     // pub owner: Address,
//     pub tokens: collections::BTreeMap<ContractTokenId, MetadataUrl>,
// }
pub struct MintParams {
    pub token_id: collections::BTreeSet<ContractTokenId>,
...
pub enum CustomContractError {
    ...
    /// Failed to mint a token because the token ID already exists.
...
impl State {
    ...
    fn mint(
        ...
        if self.tokens.get(token_id).is_some() {
            return Err(concordium_cis2::Cis2Error::Custom(
                CustomContractError::TokenAlreadyExists,
            ));
        }

        // Insert the new token and ignore the return value since you've already checked existence
        ...
        // Update token_balance or other relevant state, if needed
        Ok(())
    }
...
fn mint(
    ...
    for &token_id in params.token_id.iter() {
        fn build_token_metadata_url(params: &MintParams) -> String {
            let mut token_metadata_url = String::from(&params.metadata_url);
            token_metadata_url.push_str(&token_id.to_string());
            token_metadata_url
        }

        let metadata_url = MetadataUrl {
            url: build_token_metadata_url(&params),
            hash: None,
        };
...
fn contract_upgrade(ctx: &ReceiveContext, host: &mut LowLevelHost) -> ContractResult<()> {
    ...
    let sender = ctx.sender();
        ...
        sender.matches_account(&ctx.owner()),
        ContractError::Unauthorized
    );
